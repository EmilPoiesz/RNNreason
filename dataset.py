...
# The LogicDataset used when training RNN/GRU/LSTM
# It differs in init and collate_fn to give the bert embeddings for each 
# problem so that I can pass them directly to the model.
class RNNLogicDataset(Dataset):

    def __init__(self, examples, args=None):
        self.examples = self.curate_examples(examples, args)
        self.tokenizer  = AutoTokenizer.from_pretrained(
                            args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                            do_lower_case=args.do_lower_case,
                            cache_dir=args.cache_dir if args.cache_dir else None)
    ...
    def __getitem__(self, index):
        example = self.examples[index]
        query_facts_rules = example["query"] + " ".join(example["facts"] + example["rules"]).lower()
    
        return query_facts_rules, example["label"], example["depth"]

    def collate_fn(self, examples):
    
        batch_encoding = self.tokenizer([example[0] for example in examples],
                                         max_length=self.max_length,
                                         padding="longest",
                                         truncation=True)
        
        input_ids = torch.LongTensor(batch_encoding["input_ids"])
        attention_masks = torch.LongTensor(batch_encoding["attention_mask"])
        token_type_ids = torch.LongTensor(batch_encoding["token_type_ids"])
        inputs = {"input_ids": input_ids, "attention_mask": attention_masks, "token_type_ids": token_type_ids}  
        
        # We are passing the BERT encoded input through the BERT model
        # once without gradients so that we can get the BERT embeddings
        # as the input for the RNN models.
        with torch.no_grad():
            embeddings_ = self.embedding_model(**inputs)
        embeddings = embeddings_.last_hidden_state
        
        label_ids = torch.Tensor([example[1] for example in examples])
        depths = torch.Tensor([example[2] for example in examples])
        
        return embeddings, label_ids, depths
    ...
    def curate_examples(self, examples, args):
        if args.keep_only_negative:
            examples = [i for i in examples if i["label"] == 0]
        for index, example in enumerate(examples):
            examples[index] = self.convert_raw_example(example)
        random.shuffle(examples)
        if args.limit_example_num != -1:
            examples = examples[:args.limit_example_num]
        return examples
    ...
    def split_dataset(file_name):
        ...
        train_examples = all_examples[:len(all_examples) // 10 * 8]
        val_examples = all_examples[len(all_examples) // 10 * 8:len(all_examples) // 10 * 9]
        test_examples = all_examples[len(all_examples) // 10 * 9:]

        with open(file_name + "_train", "w") as f: json.dump(train_examples, f)
        with open(file_name + "_val", "w")   as f: json.dump(val_examples, f)
        with open(file_name + "_test", "w")  as f: json.dump(test_examples, f)
